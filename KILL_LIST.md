...
3. Mint before `node scripts/gate.js --preflight` prints PASS and the owner says mint.
...
10. Announce a mint address that `node scripts/self-audit.js` has not marked CLEAN with
    on-chain verification (never `--offline`, which refuses to print CLEAN for this reason).
...
16. Issue a credit for a reason that is not published in `EARN.md`, or at a rate that is
    not the published rate.
17. Hand-edit `config/ledger.json`, or let it disagree with chain — run
    `node scripts/reconcile.js` after every issuance.
